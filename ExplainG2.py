...
# ── Local paths (replaces Colab Google Drive paths) ──────────────────────────
ROOT      = Path(__file__).parent.resolve()   # ExplainGuardAI/
...
# Model paths — defined early so existence checks work before training
...
# ── Cell 4 ───────────────────────────────────────────────────────
# ── ECDSA Sign & Verify Helpers (Paper §3.4, Eq. 5) ─────────────────────────
# σ = Sign_sk(h2 ∥ meta)   Verify_pk(σ, h2 ∥ meta) ∈ {T, F}
...
def verify_certificate(verifying_key: VerifyingKey,
    ...
    result = {
        ...
        "h0_recompute_match": None,   # None if input_tensor not supplied
    ...
    try:
        ...
        # Step 1 (optional): recompute h0 from raw input
        ...
        # Step 2: verify h1 = SHA256(h0) — best-effort; skip if h1 is empty
        if h1_claimed and h0_claimed:
            ...
            result["h1_chain_match"] = True  # hash chain stored internally; trust sig
...
if TRANSFORMER_MODEL_PATH.exists():
    ...
    transformer_best_val_acc = 0.9952  # from saved run
...
if datasets_loaded and mean_f1:
    ...
    for row, mdl_name in enumerate(mdl_names):
        for col, ds_name in enumerate(ds_names):
            ...
            cm_norm = np.divide(cm_raw, row_sum, out=np.zeros_like(cm_raw, dtype=float), where=row_sum != 0)
            sns.heatmap(
...
# ── Paper Table 2 run configuration ─────────────────────────────────────────
# Run 1 — Transformer,   600 certs,     CICIDS2017   (integrity validation)
# Run 2 — Transformer,   431,731 certs, CICDDoS2019  (stress test)
# Run 3 — CNN-LSTM,      431,731 certs, CICDDoS2019  (architecture comparison)
# Change RUN_MODE to select which run:
#   "run1" →     600 certs, fast (~seconds)
#   "run2" → 431,731 certs, Transformer on CICDDoS2019 (~6 min)
#   "run3" → 431,731 certs, CNN-LSTM on CICDDoS2019    (~9 min)
#   "all"  → runs 1+2+3 sequentially (full paper replication)
RUN_MODE = "run1"

_RUN_CONFIG = {
    "run1": {"arch": "TabularTransformer", "n_certs": 600,    "dataset": "cicids2017"},
    "run2": {"arch": "TabularTransformer", "n_certs": 431731, "dataset": "cicddos2019"},
    "run3": {"arch": "CNN-LSTM",           "n_certs": 431731, "dataset": "cicddos2019"},
}
...
# ── Helper functions for multi-run certificate generation ────────────────────

def build_ddos_loader(batch_size=256):
    ddos_path = DATA_DIR / "cicddos2019_dataset.csv"
    if not ddos_path.exists():
        raise FileNotFoundError(f"CICDDoS2019 not found at {ddos_path}")
    print(f"  Loading CICDDoS2019...")
    df_d = pd.read_csv(ddos_path, low_memory=False, on_bad_lines="skip")
    df_d.columns = df_d.columns.str.strip()
    lc = next((c for c in df_d.columns if c.strip().lower() in
               {"label","attack_type","class","traffic_type"}), None)
    if lc is None:
        raise KeyError("No label column in CICDDoS2019")
    lab = df_d[lc].astype(str).str.strip()
    df_d = df_d[lab != "nan"].copy(); lab = lab[lab != "nan"]
    benign_kw = {"benign","normal","0"}
    y_d = np.array([0 if str(l).lower() in benign_kw else 1 for l in lab], dtype=np.int64)
    X_d = align_features(df_d, TRAIN_FEATURES)
    X_d = scaler.transform(X_d).astype(np.float32)
    print(f"  CICDDoS2019: {len(X_d):,} rows (benign={(y_d==0).sum():,} attack={(y_d==1).sum():,})")
    ds = TensorDataset(torch.from_numpy(X_d), torch.from_numpy(y_d))
    return DataLoader(ds, batch_size=batch_size, shuffle=False, num_workers=0)

def build_model_for_cert(arch_name):
    if arch_name == "TabularTransformer":
        m = TabularTransformer(input_dim=X.shape[1], hidden_dim=128, num_heads=4,
            num_layers=2, num_classes=len(label_encoder.classes_), dropout=0.1).to(DEVICE)
        m.load_state_dict(torch.load(TRANSFORMER_MODEL_PATH, map_location="cpu"))
    else:
        m = CNNLSTMHybrid(input_dim=X.shape[1], cnn_channels=128, lstm_hidden=256,
            lstm_layers=2, num_classes=len(label_encoder.classes_), dropout=0.3).to(DEVICE)
        m.load_state_dict(torch.load(CNNLSTM_MODEL_PATH, map_location="cpu"))
    m.eval(); return m

def register_cert_hooks(mdl, arch_name, cap):
    if arch_name == "TabularTransformer":
        for layer in mdl.encoder_layers:
            layer.self_attn.register_forward_hook(cap.hook_fn)
    else:
        mdl.lstm.register_forward_hook(cap.hook_fn)

def calibrate_cert_quantizer(mdl, loader, cap, qtz, n_batches=2):
    with torch.no_grad():
        for bi, (xb, _) in enumerate(loader):
            xb = xb.to(DEVICE); _ = mdl(xb)
            for li, act in enumerate(cap.activations):
                qtz.collect(act, li)
            cap.clear()
            if (bi + 1) >= n_batches: break
    qtz.finalize_calibration()

def build_fingerprint_baseline(mdl, loader, cap, expl, n=200):
    fps, cnt = [], 0
    with torch.no_grad():
        for xb, _ in loader:
            if cnt >= n: break
            ...
            for i in range(xb.size(0)):
                if cnt >= n: break
                s = xb[i:i+1]; cap.clear()
                imp = expl.explain(s); cap.clear()
                fps.append(np.array(imp).flatten()); cnt += 1
    mu    = np.mean(fps, axis=0)
    dists = [np.linalg.norm(fp - mu) for fp in fps]
    t95   = float(np.percentile(dists, 95))
    print(f"  Baseline: {cnt} samples | theta_p95={t95:.6f}")
    return mu, t95, 2*t95

def issuance_flag_fn(fp, conf, mu, t95, t2_95):
    D = float(np.linalg.norm(np.array(fp).flatten() - mu))
    flag = "NORMAL" if D <= t95 else ("WARN" if D <= t2_95 else "SUSPECT")
    if conf < 0.6: flag += "|LOW_CONFIDENCE"
    return flag, D

def run_cert_generation(run_id, arch_name, loader, n_certs, cert_dir, ds_name, mu, t95, t2_95):
    cert_dir.mkdir(parents=True, exist_ok=True)
    mdl  = build_model_for_cert(arch_name)
    cap  = ActivationCapture()
    qtz  = Quantizer(top_k=TOP_K, num_buckets=N_BUCKETS)
    expl = IntrinsicExplainer(mdl)
    ddet = DriftDetector(window_size=100, threshold=0.5)
    register_cert_hooks(mdl, arch_name, cap)
    calibrate_cert_quantizer(mdl, loader, cap, qtz, CALIBRATION_BATCHES)
    rows, generated = [], 0
    t_start = _time.perf_counter()
    print(f"\n  [{run_id}] Generating {n_certs:,} certificates | {arch_name} | {ds_name}")
    with torch.no_grad():
        for batch_idx, (xb, yb) in enumerate(loader):
            if generated >= n_certs: break
            xb = xb.to(DEVICE)
            logits = mdl(xb); probs = torch.softmax(logits, dim=1); preds = torch.argmax(probs, dim=1)
            q_acts = []
            for idx, act in enumerate(cap.activations):
                q_acts.append(qtz.quantize(act, idx) if idx in qtz.centers
                              else np.zeros((act.shape[0], qtz.top_k), dtype=np.int32))
            cap.clear()
            for i in range(xb.size(0)):
                if generated >= n_certs: break
                sample = xb[i:i+1]
                pred_id = int(preds[i].item())
                true_id = int(yb[i].item()) if hasattr(yb[i], "item") else int(yb[i])
                confidence = float(probs[i].max().item())
                # Deterministic hash chain (reproducible by verifier)
                _xb = sample.cpu().numpy().astype("float32").tobytes()
                h0 = hashlib.sha256(_xb).digest()
                h1 = hashlib.sha256(h0).digest()
                _ab = b"".join(
                    qa[i:i+1].tobytes() if hasattr(qa[i:i+1], "tobytes")
                    else bytes(qa[i:i+1].flatten().tolist()) for qa in q_acts)
                h2 = hashlib.sha256(h1 + _ab + MODEL_VERSION.encode()).digest()
                cap.clear(); importance = expl.explain(sample); cap.clear()
                if ddet.detect(importance) and (generated < 500 or generated % 50000 == 0):
                    print(f"    [DRIFT] sample {generated}")
                ddet.add(importance)
                flag, drift_score = issuance_flag_fn(importance, confidence, mu, t95, t2_95)
                signature = sign_certificate(SIGNING_KEY, h0, h1, h2, pred_id, confidence, MODEL_VERSION)
                cert = {"input_tensor": sample.cpu(), "feature_importances": importance,
                        "h0": h0, "h1": h1, "h2": h2, "signature": signature,
                        "predicted_class": pred_id, "true_class": true_id,
                        "confidence": confidence, "model_version": MODEL_VERSION,
                        "issuance_flag": flag, "drift_score": drift_score,
                        "theta_p95": t95, "run_id": run_id, "architecture": arch_name, "dataset": ds_name}
                with open(cert_dir / f"certificate_signed_{generated}.pkl", "wb") as fh:
                    pickle.dump(cert, fh)
                rows.append({"sample_id": generated, "true_class_id": true_id,
                             "pred_class_id": pred_id, "confidence": round(confidence, 6),
                             "issuance_flag": flag, "drift_score": round(drift_score, 6),
                             "theta_p95": round(t95, 6), "run_id": run_id,
                             "architecture": arch_name, "dataset": ds_name,
                             "certificate_file": f"certificate_signed_{generated}.pkl"})
                generated += 1
            if generated % max(1, n_certs // 20) == 0 or generated == n_certs:
                hz = generated / max(_time.perf_counter() - t_start, 1e-9)
                print(f"    {generated:>7,}/{n_certs:,} certs | {hz:.0f} Hz")
    df = pd.DataFrame(rows)
    df.to_csv(cert_dir / f"prediction_summary_{run_id}.csv", index=False)
    hz = generated / max(_time.perf_counter() - t_start, 1e-9)
    print(f"  [{run_id}] Complete: {generated:,} certs | {hz:.0f} Hz")
    print("  Flags:", df["issuance_flag"].value_counts().to_dict())
    return df

# ── Execute runs ──────────────────────────────────────────────────────────────
_ddos_loader = None
all_summaries = []
runs_to_execute = ["run1","run2","run3"] if RUN_MODE == "all" else [RUN_MODE]

for _run_id in runs_to_execute:
    _cfg    = _RUN_CONFIG[_run_id]
    _arch   = _cfg["arch"]
    _n      = _cfg["n_certs"]
    _ds     = _cfg["dataset"]
    _cdir   = DATA_DIR / f"certificates_{_run_id}"
    print(f"\n{'='*65}")
    print(f"  PAPER RUN {_run_id.upper()} | arch={_arch} | n={_n:,} | data={_ds}")
    print(f"{'='*65}")
    if _ds == "cicddos2019":
        if _ddos_loader is None:
            _ddos_loader = build_ddos_loader()
        _loader = _ddos_loader
    else:
        _loader = val_loader
    # Build baseline using TabularTransformer on chosen loader
    _bm  = build_model_for_cert("TabularTransformer")
    _bc  = ActivationCapture(); _bq = Quantizer(TOP_K, N_BUCKETS)
    _be  = IntrinsicExplainer(_bm)
    register_cert_hooks(_bm, "TabularTransformer", _bc)
    calibrate_cert_quantizer(_bm, _loader, _bc, _bq, CALIBRATION_BATCHES)
    _mu, _t95, _t2_95 = build_fingerprint_baseline(_bm, _loader, _bc, _be, CALIBRATION_N)
    del _bm, _bc, _bq, _be
    _sum = run_cert_generation(_run_id, _arch, _loader, _n, _cdir, _ds, _mu, _t95, _t2_95)
    all_summaries.append(_sum)

prediction_summary = pd.concat(all_summaries, ignore_index=True)
CERT_DIR   = DATA_DIR / f"certificates_{runs_to_execute[-1]}"
clean_mu   = _mu
theta_p95  = _t95
theta_2p95 = _t2_95
generated  = len(prediction_summary)
prediction_summary.to_csv(CERT_DIR / "prediction_summary_signed.csv", index=False)

print(f"\n{'='*65}")
print(f"  TOTAL CERTIFICATES ISSUED: {generated:,}")
...
print("\nPreview:")
print(prediction_summary.head(5).to_string(index=False))

# Load winner model for downstream cells
model = build_model_for_cert(winner_name)
...
def get_intrinsic_explanations(model, X_tensor, batch_size=256):
    ...
    for i in range(0, len(X_tensor), batch_size):
        batch = X_tensor[i : i + batch_size].clone().to(_device).requires_grad_(True)
...
def compute_shap(model, X_background, X_eval, device, n_background=100):
    ...
    def model_predict_np(x_np):
        t = torch.tensor(x_np, dtype=torch.float32, device=_mdev)
        with torch.no_grad():
            logits = model(t)
...
def compute_integrated_gradients(model, X_eval, device,
    ...
    _mdev = next(model.parameters()).device
    model.eval()
    X_eval = X_eval.to(_mdev)

    if baseline is None:
        baseline = torch.zeros_like(X_eval[0]).unsqueeze(0).to(_mdev)

    ig_scores = []
    for x in X_eval:
        x = x.unsqueeze(0)
        alphas = torch.linspace(0, 1, n_steps, device=_mdev)
...
def main():
    ...
    if winner_name == "TabularTransformer":
        ...
        model = cnnlstm_model
    model.eval()
    # Always use the device the model is actually on, not the CLI arg
    device = next(model.parameters()).device
    print(f"[model] using trained {winner_name} from pipeline (device={device})")
